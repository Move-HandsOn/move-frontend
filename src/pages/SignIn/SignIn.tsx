...
function SignIn() {
  ...
  const dataLoginValidSchema = zod.object({
		...
		password: zod.string().min(8, "Preencha todos os campos").max(24,"Senha grande demais"),
	});
  ...
  const { mutateAsync: loginAsync, error, isError } = useMutation({
  ...
  console.log(error);
  return (
    <form onSubmit={handleSubmit(handleLogin)} className={style.signIn_container}>
      ...
      {errors.email && errors.email.message && <p className={style.error}>{errors.email.message}</p>}
      {isError && error.message && <p className={style.error}>{errorMessages[error.message]}</p>}
      <InputPassword {...register("password")} />
      {errors.password && errors.password.message && <p className={style.error}>{errors.password.message}</p>}
